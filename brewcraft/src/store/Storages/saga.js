...
function* fetchStorages() {
  try {
    ...
    yield put({ type: FETCH_STORAGES_SUCCESS, payload: response.data.content });
  } catch (e) {
...
async function addStorageRequest(payload) {
  return await AxiosInstance.post(`/api/v1/facilities/${42}/storages`, payload);
}
...
async function editStorageItemRequest(id, data) {
  return await AxiosInstance.patch(`/api/v1/facilities/storages/${id} `, data);
}
...
function* deleteStorage(action) {
  try {
    ...
    yield put({ type: DELETE_STORAGES_SUCCESS, payload: action.payload.id });
    action.payload.success && action.payload.success();
    yield put(snackSuccess());
  } catch (e) {
