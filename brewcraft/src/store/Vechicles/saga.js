import { call, put, takeLatest } from "redux-saga/effects";
import {
  FETCH_VECHICLE_REQUEST,
  FETCH_VECHICLE_SUCCESS,
  FETCH_VECHICLE_FAILURE,
  ADD_VECHICLE_REQUEST,
  ADD_VECHICLE_SUCCESS,
  ADD_VECHICLE_FAILURE,
  EDIT_VECHICLE_REQUEST,
  EDIT_VECHICLE_SUCCESS,
  EDIT_VECHICLE_FAILURE,
  DELETE_VECHICLE_REQUEST,
  DELETE_VECHICLE_SUCCESS,
} from "./actionTypes";
import {attempt,get, omit} from 'lodash'


const fetchrequest = function () {
  
  return true;
};

function* fetchVechicle() {
  const result = yield call(fetchrequest);
}


function* addVechicle(action) {
  
  let dailogStatus = get(action,'payload');

 
  const id  = yield Math.random();
  yield put({
    type: ADD_VECHICLE_SUCCESS,
    payload: { ...omit(dailogStatus, ['dialog']), id, image:"data:image/jpeg;base64," },
  });
  yield call(get(action,'payload.dialog'), id );
 
}

function* editVechicle(action) {

  yield put({ type: EDIT_VECHICLE_SUCCESS, payload: { ...omit(action.payload, "dialog")} });
  attempt(get(action,"payload.dialog"))
}


function* deleteVechicle(action) {

  yield put({ type: DELETE_VECHICLE_SUCCESS, payload: { ...action.payload } });

}

export default function* Vechicles() {
  yield takeLatest(FETCH_VECHICLE_REQUEST, fetchVechicle);
  yield takeLatest(ADD_VECHICLE_REQUEST, addVechicle);
  yield takeLatest(EDIT_VECHICLE_REQUEST, editVechicle);
  yield takeLatest(DELETE_VECHICLE_REQUEST, deleteVechicle);
}
