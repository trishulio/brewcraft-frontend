...
function* fetchAllSupplierContactsGenerator() {
    try {
        ...
        yield put({ type: FETCH_ALL_SUPPLIER_CONTACTS_SUCCESS, data: { data: res.data }});
    } catch (e) {
...
function* fetchSupplierContactsGenerator(action) {
    try {
      const res = yield call(api.fetchSupplierContacts, get(action, "payload.params"));
      delete Object.assign(res.data, {content: res.data["supplierContacts"] })["supplierContacts"];
      yield put({ type: FETCH_SUPPLIER_CONTACTS_SUCCESS, data: { data: res.data }});
    } catch (e) {
...
function* SupplierContacts() {
    yield takeEvery(FETCH_SUPPLIER_CONTACTS_REQUEST, fetchSupplierContactsGenerator);
    yield takeEvery(FETCH_ALL_SUPPLIER_CONTACTS_REQUEST, fetchAllSupplierContactsGenerator);
}
