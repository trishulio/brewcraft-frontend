...
function* fetchIngredientByIdGenerator(action) {
    try {
        const res = yield call(api.fetchIngredientById,get(action, "payload.id"));
        res.initial = JSON.parse(JSON.stringify(res.data));
        yield put({ type: SET_INGREDIENT_DETAILS, payload: { data: res.data, initial: res.data }});
...
function* Ingredient() {
    yield takeEvery(FETCH_INGREDIENT_BY_ID_REQUEST, fetchIngredientByIdGenerator);
