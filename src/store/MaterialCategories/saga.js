...
function* fetchAllCategoriesGenerator() {
    try {
        ...
        yield put({ type: FETCH_ALL_MATERIAL_CATEGORIES_SUCCESS, data: { data: res.data }});
    } catch (e) {
...
function* fetchMaterialCategoriesGenerator(action) {
    try {
      const res = yield call(api.fetchMaterialCategories, get(action, "payload.params"));
      yield put({ type: FETCH_MATERIAL_CATEGORIES_SUCCESS, data: { data: res.data }});
    } catch (e) {
...
function* MaterialCategories() {
    yield takeEvery(FETCH_MATERIAL_CATEGORIES_REQUEST, fetchMaterialCategoriesGenerator);
    yield takeEvery(FETCH_ALL_MATERIAL_CATEGORIES_REQUEST, fetchAllCategoriesGenerator);
}
