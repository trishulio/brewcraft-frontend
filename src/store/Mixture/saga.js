...
function* fetchMashMixtureByIdGenerator(action) {
    try {
        ...
        yield put({ type: SET_MASH_MIXTURE_DETAILS, payload: { data: res.data, initial: res.data }});
    } catch (e) {
        yield put(snackFailure("Something went wrong please try again."));
    }
}
...
function* deleteMashMixtureGenerator(action) {
    try {
        ...
        yield put({ type: DELETE_MASH_MIXTURE_SUCCESS , payload : get(action, "payload") });
...
function* deleteKettleMixtureGenerator(action) {
    try {
        yield call(api.deleteMixture, get(action, "payload.id"));
        yield put({ type: DELETE_KETTLE_MIXTURE_SUCCESS , payload : get(action, "payload") });
        yield put(snackSuccess());
    } catch (e) {
...
function* fetchWhirlpoolMixtureByIdGenerator(action) {
    try {
        const res = yield call(api.fetchMixtureById,get(action, "payload.id"));
        res.initial = JSON.parse(JSON.stringify(res.data));
        yield put({ type: SET_WHIRLPOOL_MIXTURE_DETAILS, payload: { data: res.data, initial: res.data }});
    } catch (e) {
        yield put(snackFailure("Something went wrong please try again."));
    }
}
...
function* deleteWhirlpoolMixtureGenerator(action) {
    try {
        yield call(api.deleteMixture, get(action, "payload.id"));
        yield put({ type: DELETE_WHIRLPOOL_MIXTURE_SUCCESS , payload : get(action, "payload") });
        yield put(snackSuccess());
    } catch (e) {
...
function* Mixture() {
    ...
    yield takeEvery(FETCH_MASH_MIXTURE_BY_ID_REQUEST, fetchMashMixtureByIdGenerator);
    ...
    yield takeEvery(FETCH_KETTLE_MIXTURE_BY_ID_REQUEST, fetchKettleMixtureByIdGenerator);
    ...
    yield takeEvery(DELETE_KETTLE_MIXTURE_REQUEST, deleteKettleMixtureGenerator);
    yield takeEvery(FETCH_WHIRLPOOL_MIXTURE_BY_ID_REQUEST, fetchWhirlpoolMixtureByIdGenerator);
    yield takeEvery(ADD_WHIRLPOOL_MIXTURE_REQUEST, addWhirlpoolMixtureGenerator);
    yield takeEvery(EDIT_WHIRLPOOL_MIXTURE_REQUEST, editWhirlpoolMixtureGenerator);
    yield takeEvery(DELETE_WHIRLPOOL_MIXTURE_REQUEST, deleteWhirlpoolMixtureGenerator);
}
